...
platform=$(getprop ro.hardware.soc.manufacturer)
if [ "$platform" == "mtk" ] && [ -e /dev/block/by-name/expdb ] ; then
	cat /dev/block/by-name/expdb > /data/vendor/dontpanic/expdb
	chown root:log /data/vendor/dontpanic/expdb
	chmod 0640 /data/vendor/dontpanic/expdb
fi
